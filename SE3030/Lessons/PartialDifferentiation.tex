...
\frame{{Introduction}
	...
	\end{block}
    %TODO: Make sure I know what I'm talking about.
	...
	\begin{exampleblock}{4.1.12}
		\begin{align*}
			z &= x^{2}+2y^{2}, x=r\text{cos}\theta, y=r\text{sin}\theta \\
	    		&= x^{2}+2y^{2} \\
	    		&= r^{2}\text{cos}^{2}\theta+2r^{2}\text{sin}^{2}\theta \\
	    		\left(\dfrac{\partial z}{\partial y}\right)_{\theta}  &= 2r\text{cos}^{2}\theta+4r\text{sin}^{2}\theta \\
			&= 2r\left(\text{cos}^{2}\theta+2r\text{ sin}^{2}\theta\right) 
		\end{align*}
	\end{exampleblock}
}
