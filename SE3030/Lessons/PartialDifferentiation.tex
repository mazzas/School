...
\frame{{Example}
    %TODO: This example is wrong.  Fix or replace.
...
\frame{{Chain Rule (Redux) Example}
    %TODO: Check correctness of this example.
	\begin{exampleblock}{Boas, 4.7.3}
		\begin{align*}
			\text{Given: } z &= xe^{-y} \\
			x &= \text{cosh} t \\
			y &= \text{cos} s \\
			\text{Differentiate: } dz &= x^2dy + e^{-y}dx \\
			dx &= 0 \\
			dy &= 0 \\
			\text{Substitute: } dz &= x^2\cdot 0 + e^{-y}\cdot 0 \\
			&= 0
		\end{align*}
	\end{exampleblock}
}
