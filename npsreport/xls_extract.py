...
dhs_categories = [("Direct Labor",'TOTAL FACULTY/SUPPORT Labor.*'),
                  ('Travel','Travel'),
                  ('Equipment','Equipment'),
                  ('Contracts - Services','2.*Contracts'),
...
def xls_date(num):
    ...
    import datetime
    v = datetime.datetime(1900,1,1)+datetime.timedelta(days=num-2)
...
def extract_from_xls(ifn,ofn):
    ...
    set_variable("proposalTitle",year1.row(5)[0].value.strip())
    set_variable("sponsor",year1.row(5)[6].value.strip())
    ...
    def find_value(sheet,title,colmax=1):
    ...
    POPStart = find_value(yearN,"From:",colmax=7)
    POPStartStr = xls_date(POPStart)
    print("POPStart: {} ({})".format(POPStart,POPStartStr))
    if type(POPStart)!=float:
        print type(POPStart)
        print "Proposal Start period not set on total page"
        exit(1)


    POPEnd = find_value(yearN,"To:",colmax=7)
    print("POPEnd: {} ({})".format(POPEnd,xls_date(POPEnd)))
    if type(POPEnd)!=float:
        print "Proposal End not set on total page"
        exit(1)

    set_variable("POPStart",xls_date(POPStart))
    set_variable("POPEnd",xls_date(POPEnd))
    set_variable("POPMonths",int((POPEnd-POPStart)/30))
    ...
    # Figure out the current year budget
    if str(current_fy) in POPStartStr:
